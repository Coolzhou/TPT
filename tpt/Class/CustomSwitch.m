...
- (void)drawRect:(CGRect)rect
{
    if (!first) {
        ...
        _customSwitch.backgroundColor = [UIColor orangeColor];
        [_customSwitch setFrame:CGRectMake(0, 0,self.bounds.size.width,self.bounds.size.height)];
        [_onButton setBackgroundImage:_onImage forState:UIControlStateNormal];
        [_offButton setBackgroundImage:_offImage forState:UIControlStateNormal];
...
//        [_offButton setImage:_offImage forState:UIControlStateNormal];
        ...
        _leftRect = CGRectMake(-(self.bounds.size.width - self.bounds.size.height), 0, self.bounds.size.width, self.bounds.size.height);
        _middleRect = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height);
        ...
        _minusTranslate = self.bounds.size.width - self.bounds.size.height;
        _currentTranslationX = 0;
        if (_arrange == CustomSwitchArrangeONLeftOFFRight) {

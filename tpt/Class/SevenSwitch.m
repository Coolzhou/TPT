...
@interface SevenSwitch ()  {
    ...
    UILabel *onTitleLable;
    UILabel *ofTitleLable;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (CGRectIsEmpty(frame)) {
        initialFrame = CGRectMake(0, 0, 60, 25);
    }
    else {
        initialFrame = frame;
    }
...
- (void)setup {
    ...
    if (self.selectType ==SevenSwitchImageType) {
        // images
        ...
        // images
        onTitleLable = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width - self.frame.size.height, self.frame.size.height)];
        onTitleLable.alpha = 0;
        onTitleLable.font = [UIFont systemFontOfSize:16];
        onTitleLable.textColor = [UIColor whiteColor];
        onTitleLable.textAlignment = NSTextAlignmentCenter;
        [background addSubview:onTitleLable];

        ofTitleLable = [[UILabel alloc] initWithFrame:CGRectMake(self.frame.size.height, 0, self.frame.size.width - self.frame.size.height, self.frame.size.height)];
        ofTitleLable.alpha = 1.0;
        ofTitleLable.font = [UIFont systemFontOfSize:16];
        ofTitleLable.textColor = [UIColor whiteColor];
        ofTitleLable.textAlignment = NSTextAlignmentCenter;
        [background addSubview:ofTitleLable];
    }
    // knob
...
- (void)layoutSubviews {
    ...
    if (!isAnimating) {
        ...
        if (self.selectType == SevenSwitchImageType) {
            ...
            // images
            onTitleLable.frame = CGRectMake(0, 0, frame.size.width - frame.size.height, frame.size.height);
            ofTitleLable.frame = CGRectMake(frame.size.height, 0, frame.size.width - frame.size.height, frame.size.height);
        }
        // knob
...
-(void)setOnTitleStr:(NSString *)onTitleStr{
    onTitleStr = onTitleStr;
    onTitleLable.text = onTitleStr;
}

-(void)setOffTitleStr:(NSString *)offTitleStr{
    offTitleStr = offTitleStr;
    ofTitleLable.text = offTitleStr;
}
    ...
    if (animated) {
        ...
        [UIView animateWithDuration:0.3 delay:0.0 options:UIViewAnimationOptionCurveEaseOut|UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            if (self.selectType ==SevenSwitchImageType) {
                ...
                onTitleLable.alpha = 1.0;
                ofTitleLable.alpha = 0;
            }
...
-(void)setSelectType:(SevenSwitchType)selectType{
